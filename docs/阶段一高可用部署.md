...
- 如果某条 assignment 连续 `reconcile` 失败达到阈值，系统会把它自动切到 `paused`，停止自动恢复，等待运维显式 `resume`
...
- 只有在长时间无进展、assignment 持续 `error` / `paused`、或 internal 网络 / 鉴权配置异常时，才需要人工介入
...
| standby 长时间无进展，或 assignment 持续 `error` / `paused` | 自动恢复链路未能收敛，或已达到自动暂停阈值 | 这时才排查 internal 地址、shared secret、网络、鉴权，并视情况执行 `retry` / `resume` / 手工触发 |
...
# 将 error assignment 重新推进到 pending
...
- `ha assignments status` 当前还会显示 `failureCount` 和 `nextRetryAt`，可直接观察连续失败次数与下一次自动恢复时间
...
- 总体方案：[多实例与多副本方案设计.md](./多实例与多副本方案设计.md)
- 详细设计：[内部复制版备用节点设计.md](./内部复制版备用节点设计.md)
- 实施清单：[内部复制实施清单.md](./内部复制实施清单.md)
