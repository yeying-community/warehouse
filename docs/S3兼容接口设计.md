...
6. 第一阶段只开放 `personal`、`apps` 和 `services` 三类逻辑 bucket。
...
https://s3.yeying.pub/services/reporting/daily.json
```
...
| `services` | `/<user_root>/services/` | 支持 |
...
- 绑定路径与 `/services` 相交时，可以看到 `services` bucket。
...
| CreateBucket | `PUT /{bucket}` | 对 `personal`、`apps`、`services` 返回幂等成功，不创建新 bucket |
...
- [ ] 实现 `personal`、`apps`、`services` bucket。
...
5. 第一阶段支持 `personal`、`apps` 和 `services`，分享延后。
