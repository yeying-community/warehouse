...
本文回答“系统由哪些模块组成、启动时如何装配、运行时依赖关系是什么”。

以下内容不在本文重复展开：

- WebDAV 单次请求在权限、配额、回收站上的细粒度处理顺序：见 [WebDAV请求处理流程.md](./WebDAV请求处理流程.md)
- 认证方式、UCAN、访问密钥：见 [认证设计.md](./认证设计.md)
- 具体接口字段：见 [WebDAV与相关接口API（简明版）.md](./WebDAV与相关接口API（简明版）.md)

## 目标与非目标

- 目标：解释组件之间的依赖关系、核心运行时链路、模块划分。
- 非目标：接口细节与字段含义（详见 [WebDAV与相关接口API（简明版）.md](./WebDAV与相关接口API（简明版）.md)）。
