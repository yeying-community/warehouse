...
- 运行时复制链路在接入 control-plane assignment 后，不再因为 `peer_node_id` / `cluster_nodes` 自动推导复制目标
...
- [x] active 通过共享控制面发现 peer URL，显式 CLI / 配置覆盖路径仍保留
...
- [x] active / standby 的运行时 peer resolver 只按 effective assignment 解析 peer，不再在有 control-plane assignment 时自动回退到 `peer_node_id` / `cluster_nodes`
...
- assignment allocator / renewer、startup/manual reconcile、增量 outbox 路径都已收紧到有效 assignment；但 `peer_node_id` / `peer_base_url` 在本地调试与显式 CLI 场景下仍然保留
