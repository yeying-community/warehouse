...
- `1 active + N standby candidates，1 个 effective standby`
...
- active 会周期性在 `cluster_replication_assignments` 中维持 1 条有效 assignment
- 当前只有 assignment 指向的 effective standby 会接收历史补齐和日常增量复制
...
这是一种“单活 + 多候选 standby + 单生效副本”的方案，不是“一个 active 同时向多个 standby 正式 fan-out”的多副本方案。

### 1.1 快速结论

先看三句话：

- 可以注册多个 standby，但当前同一时刻只会有 1 个 effective standby 真正参与复制
- `cluster_nodes` 负责“发现候选节点”，`cluster_replication_assignments` 负责“决定当前复制给谁”
- outbox / worker / reconcile / offset 现在都还是单目标模型，所以当前真实能力不是 `1 active + N effective standby`
...
flowchart LR
  ...
  SA[Standby候选A] -->|heartbeat + advertise_url| PG
  SB[Standby候选B] -->|heartbeat + advertise_url| PG
  SN[Standby候选N] -->|heartbeat + advertise_url| PG

  PG -->|effective assignment| A
  A -->|reconcile + outbox增量| ES[当前effective standby]
  ES --> SD[(Standby本地数据盘)]

  SA -. 非当前assignment，只注册不接收复制 .-> PG
  SB -. 非当前assignment，只注册不接收复制 .-> PG
  SN -. 非当前assignment，只注册不接收复制 .-> PG
```
...
| 项目 | 当前实现 |
| --- | --- |
| 拓扑 | `1 active + N standby candidates，1 个 effective standby` |
...
| 复制目标选择 | 只认 effective assignment |
| 地址解析 | 从 `cluster_nodes.advertise_url` 解析 |
| 历史补齐 | active 启动时自动 reconcile，也可手工触发 |
| 日常增量 | `replication_outbox` -> worker -> 当前 effective standby |
| 当前硬边界 | 还不支持一个 active 同时向多个 standby fan-out |
...
- active 是唯一对外写入的主实例
- standby 是只接 internal 同步流量的被动副本集合，但同一时刻只有 1 个 effective standby 真正参与复制
- PostgreSQL 不只存业务元数据，也承担“共享控制面”
- active 先把本地文件写成功，再把变化记到 outbox
- 当前 effective standby 通过“历史补齐 + 后续增量”维持第二份本地数据
- assignment / generation / fence 负责保证“当前到底谁能往哪个 standby 写”

如果只记一件事，就是：

- `cluster_nodes` 解决“有哪些 standby 候选在线、地址是什么”
- `cluster_replication_assignments` 解决“当前 active 实际复制给谁、当前在哪一代”
- `replication_outbox` / `replication_offsets` 解决“当前 effective standby 已经复制到哪里了”
...
- 但正常情况下，只有被 assignment 选中的那个 effective standby 会真正接收复制流量
- standby 只做三件事：
  - 上报自身心跳
  - 接收 active 发来的复制请求
  - 维护本地复制进度与状态
...
- 当前代码支持“多个 candidate”，但只支持“一个 effective standby”
...
- 当前阶段它还是单 active -> 单 effective standby 模型的分配器，不是多副本调度器
