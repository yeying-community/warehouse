...
- `node`：节点身份、角色与 `advertise_url`
- `replication`：internal 复制开关、shared secret、退避参数、自动暂停阈值
...
- 部署时重点确认：
  ...
  - 如果启用阶段一复制，建议显式确认 `replication.reconcile_auto_pause_failures` 是否符合预期

### Replication 配置提醒

在 `1 active + N standby` 场景下，建议重点检查：

- `node.id`：每个实例必须唯一
- `node.role`：只能是 `active` 或 `standby`
- `node.advertise_url`：必须是其他实例能访问到的 internal 地址
- `replication.shared_secret`：active / standby 必须一致
- `replication.retry_backoff_base` / `replication.max_retry_backoff`：同时影响 outbox 重试和 assignment error 自动恢复节奏
...
- 如果是正式环境发布，建议优先走安装包部署，具体步骤见：
  - [安装包部署文档.md](./安装包部署文档.md)
