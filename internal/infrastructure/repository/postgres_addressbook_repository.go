...
type AddressBookRepository interface {
	...
	GetVisibleGroupByID(ctx context.Context, userID, walletAddress, groupID string) (*addressbook.Group, error)
...
func (r *PostgresAddressBookRepository) GetVisibleGroupByID(ctx context.Context, userID, walletAddress, groupID string) (*addressbook.Group, error) {
	query := `
	...
	`
	group := &addressbook.Group{}
	err := r.db.QueryRowContext(ctx, query, groupID, userID, strings.TrimSpace(walletAddress), addressbook.MemberStatusActive).Scan(
		&group.ID,
		&group.UserID,
		&group.Name,
		&group.CreatedAt,
...
func (r *PostgresAddressBookRepository) ListVisibleGroups(ctx context.Context, userID, walletAddress string) ([]*addressbook.Group, error) {
	...
	`
	rows, err := r.db.QueryContext(ctx, query, userID, strings.TrimSpace(walletAddress), addressbook.MemberStatusActive)
	if err != nil {
...
func (r *PostgresAddressBookRepository) CreateMember(ctx context.Context, member *addressbook.Member) error {
	...
	_, err := r.db.ExecContext(ctx, query,
		...
		addressbook.NormalizeMemberStatus(member.Status),
		member.CreatedAt,
	)
	if err != nil {
...
func (r *PostgresAddressBookRepository) GetMemberByID(ctx context.Context, userID, memberID string) (*addressbook.Member, error) {
	...
	member.Tags = tags
	member.Status = addressbook.NormalizeMemberStatus(member.Status)
	return member, nil
}
...
func (r *PostgresAddressBookRepository) UpdateMember(ctx context.Context, member *addressbook.Member) error {
	...
	`
	result, err := r.db.ExecContext(ctx, query, member.GroupID, member.Name, member.WalletAddress, pq.Array(member.Tags), addressbook.NormalizeMemberStatus(member.Status), member.ID, member.UserID)
	if err != nil {
		if isDuplicateMemberError(err) {
			return addressbook.ErrDuplicateMember
		}
...
func (r *PostgresAddressBookRepository) UpdateMemberStatus(ctx context.Context, userID, memberID, status string) error {
	query := `UPDATE group_members SET status = $1 WHERE id = $2 AND user_id = $3`
	result, err := r.db.ExecContext(ctx, query, addressbook.NormalizeMemberStatus(status), memberID, userID)
	if err != nil {
